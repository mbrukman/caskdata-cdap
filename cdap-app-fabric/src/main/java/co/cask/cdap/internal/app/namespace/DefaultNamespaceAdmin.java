...
public final class DefaultNamespaceAdmin implements NamespaceAdmin {
  ...
  private void createDefaultNamespace() {
    ...
    try {
      ...
      // propagating it as an unchecked exception because this will be gone soon via CDAP-1213,
  ...
  public void createNamespace(NamespaceMeta metadata) throws AlreadyExistsException, IOException {
    ...
    String namespaceId = metadata.getId();
    ...
    if (namespaceLocation.exists()) {
      // need this hack because default namespace cannot be deleted, so the directory is always found, once created
      // TODO: CDAP-1213 - Remove this hack once default namespace is created at a more deterministic time.
    ...
    if (!namespaceLocation.mkdirs()) {
      // need this hack because default namespace cannot be deleted, so the directory is always found, once created
      // will be removed once default namespace is created at a more deterministic time.
      if (!Constants.DEFAULT_NAMESPACE.equals(namespaceId)) {
